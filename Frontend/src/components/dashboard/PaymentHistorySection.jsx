...
const PaymentHistorySection = () => {
  ...
  const fetchPatients = async () => {
    try {
      ...
      const response = await fetch(`${getBackendUrl()}/api/patients`, {
  ...
  const fetchPaymentHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${getBackendUrl()}/api/paiements/historique-complet`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      if (response.ok) {
        ...
        console.log(
          "Endpoint historique complet non disponible, utilisation alternative",
        );
        setPaymentHistory([]);
        setFilteredHistory([]);
      }
  ...
  useEffect(() => {
    ...
    if (dateFilter) {
      filtered = filtered.filter(
        (item) => item.created_at?.split("T")[0] === dateFilter,
      );
    }
  ...
  const getTypePaiementDisplay = (type, chequeInfo) => {
    if (type === "espece") {
      ...
      if (chequeInfo) {
        try {
          const info =
            typeof chequeInfo === "string"
              ? JSON.parse(chequeInfo)
              : chequeInfo;
  ...
  const stats = {
    ...
    total_montant: paymentHistory.reduce(
      (sum, item) =>
        sum + (parseFloat(item.montant_total_apres || item.montant_total) || 0),
      0,
    ),
    total_paye: paymentHistory.reduce(
      (sum, item) =>
        sum + (parseFloat(item.montant_paye_apres || item.montant_paye) || 0),
      0,
    ),
    total_restant: paymentHistory.reduce(
      (sum, item) =>
        sum +
        (parseFloat(item.montant_restant_apres || item.montant_restant) || 0),
      0,
    ),
  };
    ...
    <div>
      ...
      <div style={{ marginBottom: "25px" }}>
        ...
        >
          ...
          >
            <div style={{ fontSize: "24px", fontWeight: "bold" }}>
              {stats.total_montant.toFixed(2)} DT
            </div>
          ...
          >
            <div style={{ fontSize: "24px", fontWeight: "bold" }}>
              {stats.total_restant.toFixed(2)} DT
            </div>
      ...
      >
        <div
          style={{ display: "flex", gap: "15px", flex: 1, flexWrap: "wrap" }}
        >
              ...
              {loading ? (
                ...
                currentItems.map((item, index) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px" }}>
                      {getTypePaiementDisplay(
                        item.type_paiement_apres || item.type_paiement,
                        item.cheque_info_apres || item.cheque_info,
                      )}
                    </td>
      ...
      {showDetailsModal && selectedPatient && (
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: "10px" }}>
              <strong>Montant avant modification:</strong>{" "}
              {selectedPatient.montant_total_avant?.toFixed(2) || 0} DT
            </div>
            <div style={{ marginBottom: "10px" }}>
              <strong>Montant après modification:</strong>{" "}
              {selectedPatient.montant_total_apres?.toFixed(2) ||
                selectedPatient.montant_total?.toFixed(2) ||
                0}{" "}
              DT
            </div>
            <div style={{ marginBottom: "10px" }}>
              <strong>Montant payé:</strong>{" "}
              {selectedPatient.montant_paye_apres?.toFixed(2) ||
                selectedPatient.montant_paye?.toFixed(2) ||
                0}{" "}
              DT
            </div>
            <div style={{ marginBottom: "10px" }}>
              <strong>Montant restant:</strong>{" "}
              {selectedPatient.montant_restant_apres?.toFixed(2) ||
                selectedPatient.montant_restant?.toFixed(2) ||
                0}{" "}
              DT
            </div>
            <div style={{ marginBottom: "10px" }}>
              ...
              {getStatutBadge(
                selectedPatient.statut_apres || selectedPatient.statut,
              )}
            </div>
            <div style={{ marginBottom: "10px" }}>
              <strong>Mode paiement:</strong>{" "}
              {getTypePaiementDisplay(
                selectedPatient.type_paiement_apres ||
                  selectedPatient.type_paiement,
                selectedPatient.cheque_info_apres ||
                  selectedPatient.cheque_info,
              )}
            </div>
            <div style={{ marginBottom: "10px" }}>
