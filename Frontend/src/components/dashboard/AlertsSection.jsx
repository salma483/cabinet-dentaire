...
import { FaBell, FaCalendarAlt, FaCheckCircle, FaTimesCircle, FaEnvelope, FaTrash, FaMoneyBillWave, FaExclamationTriangle, FaDatabase } from 'react-icons/fa';
...
const AlertsSection = ({ appointments, patients, onPaymentClick }) => {
  ...
  const getActualStatus = (patient) => {
    ...
    const reste = patient.montant_restant !== undefined ? parseFloat(patient.montant_restant) : (total - paye);
  ...
  const generateLocalAlerts = useCallback(() => {
    ...
    if (alertSettings.paymentReminder && alertSettings.showSemiPaye && semiPayePatients.length > 0) {
      console.log(`📢 Génération alertes semi-payés: ${semiPayePatients.length} patients`);
      ...
      semiPayePatients.forEach(patient => {
        ...
        const montantRestant = patient.montant_restant !== undefined 
    ...
    if (alertSettings.paymentReminder && alertSettings.showNonPaye && nonPayePatients.length > 0) {
      console.log(`🔴 Génération alertes non-payés: ${nonPayePatients.length} patients`);
    ...
    console.log(`📋 Total alertes générées: ${newAlerts.length} (Semi: ${newAlerts.filter(a => a.type === 'semi_payment').length}, Non-payé: ${newAlerts.filter(a => a.type === 'unpaid').length})`);
  ...
  const fetchDbAlerts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/paiements/alertes', {
      ...
      if (response.ok) {
        ...
        const formattedDbAlerts = data.map(alert => {
          ...
          const isSemiPaye = alert.statut === 'semi_paye' || (!isCritical && alert.montant_restant > 0);
          
          return {
            id: `db_${alert.id}`,
            dbId: alert.id,
            type: isCritical ? 'unpaid' : 'semi_payment',
            title: isCritical ? '🔴 ALERTE CRITIQUE - Paiement Impayé' : '⚠️ ALERTE - Paiement Partiel (Semi-payé)',
  ...
  const mergeAlerts = useCallback(() => {
    ...
    if (alertSettings.showDbAlerts) {
      const filteredDbAlerts = dbAlerts.filter(alert => {
        if (alert.type === 'semi_payment' && !alertSettings.showSemiPaye) return false;
    ...
    uniqueAlerts.sort((a, b) => (priorityOrder[a.priority] || 2) - (priorityOrder[b.priority] || 2));
    
    console.log(`🔄 Alertes fusionnées: ${uniqueAlerts.length} (Semi: ${uniqueAlerts.filter(a => a.type === 'semi_payment').length}, Non-payé: ${uniqueAlerts.filter(a => a.type === 'unpaid').length})`);
  ...
  const markDbAlertAsRead = async (alertId, dbId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/paiements/alertes/${dbId}/lire`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      if (response.ok) {
        setDbAlerts(prev => prev.map(alert => 
          alert.id === alertId ? { ...alert, read: true, status: 'lue' } : alert
        ));
  ...
  const deleteDbAlert = async (alertId, dbId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/paiements/alertes/${dbId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const getAlertStyle = (alert) => {
    if (alert.read) return { background: 'white', borderLeft: '4px solid #dee2e6' };
    switch(alert.type) {
      case 'unpaid':
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <h4 style={{ margin: 0 }}>
          ...
          {unreadCount > 0 && (
            <span style={{ marginLeft: '10px', background: '#dc3545', color: 'white', padding: '2px 8px', borderRadius: '20px', fontSize: '12px' }}>
              {unreadCount} nouvelle(s)
            </span>
          )}
        </h4>
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={fetchDbAlerts} disabled={loading} style={{ padding: '8px 16px', background: '#28a745', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
            <FaDatabase /> Actualiser
          </button>
          <button onClick={() => setShowSettings(!showSettings)} style={{ padding: '8px 16px', background: '#667eea', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
            ⚙️ Paramètres
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '20px' }}>
        <div style={{ background: '#d4edda', padding: '15px', borderRadius: '10px', borderLeft: '4px solid #28a745' }}>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#155724' }}>{paymentStats.payeCount}</div>
          <div style={{ fontSize: '12px', color: '#155724' }}><FaCheckCircle /> Patients payés</div>
        </div>
        <div style={{ background: '#fff3cd', padding: '15px', borderRadius: '10px', borderLeft: '4px solid #ffc107' }}>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#856404' }}>{paymentStats.semiPayeCount}</div>
          <div style={{ fontSize: '12px', color: '#856404' }}><FaExclamationTriangle /> Semi-payés</div>
        </div>
        <div style={{ background: '#f8d7da', padding: '15px', borderRadius: '10px', borderLeft: '4px solid #dc3545' }}>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#721c24' }}>{paymentStats.nonPayeCount}</div>
          <div style={{ fontSize: '12px', color: '#721c24' }}><FaTimesCircle /> Non payés</div>
        </div>
        <div style={{ background: '#cce5ff', padding: '15px', borderRadius: '10px', borderLeft: '4px solid #004085' }}>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#004085' }}>{paymentStats.totalRestant.toFixed(2)} DT</div>
          <div style={{ fontSize: '12px', color: '#004085' }}><FaMoneyBillWave /> Total restant</div>
        </div>
      </div>
      ...
      {showSettings && (
        <div style={{ background: 'white', borderRadius: '15px', padding: '20px', marginBottom: '20px' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
            <input type="checkbox" checked={alertSettings.paymentReminder} onChange={(e) => setAlertSettings(prev => ({ ...prev, paymentReminder: e.target.checked }))} style={{ marginRight: '10px' }} />
            <span>💰 Activer les alertes de paiement</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', marginBottom: '10px', marginLeft: '25px' }}>
            <input type="checkbox" checked={alertSettings.showSemiPaye} onChange={(e) => setAlertSettings(prev => ({ ...prev, showSemiPaye: e.target.checked }))} style={{ marginRight: '10px' }} />
            <span><FaExclamationTriangle color="#ffc107" /> Afficher les alertes SEMI-PAYÉS</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', marginBottom: '10px', marginLeft: '25px' }}>
            <input type="checkbox" checked={alertSettings.showNonPaye} onChange={(e) => setAlertSettings(prev => ({ ...prev, showNonPaye: e.target.checked }))} style={{ marginRight: '10px' }} />
            <span><FaTimesCircle color="#dc3545" /> Afficher les alertes NON-PAYÉS</span>
          </label>
          <hr />
          <label style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
            <input type="checkbox" checked={alertSettings.showDbAlerts} onChange={(e) => setAlertSettings(prev => ({ ...prev, showDbAlerts: e.target.checked }))} style={{ marginRight: '10px' }} />
            <span>🗄️ Afficher alertes de la base de données</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
            <input type="checkbox" checked={alertSettings.showLocalAlerts} onChange={(e) => setAlertSettings(prev => ({ ...prev, showLocalAlerts: e.target.checked }))} style={{ marginRight: '10px' }} />
      ...
      <div style={{ background: 'white', borderRadius: '15px', overflow: 'hidden' }}>
        {alerts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px', color: '#6c757d' }}>
            ...
            <small>Vérifiez que vous avez des patients avec statut "Semi-payé" ou "Non payé"</small>
          </div>
        ) : (
          <>
            {/* Résumé des alertes */}
            <div style={{ padding: '15px 20px', background: '#f8f9fa', borderBottom: '1px solid #dee2e6', display: 'flex', gap: '20px' }}>
              <span>📊 Total: {alerts.length} alertes</span>
              <span style={{ color: '#ffc107' }}>⚠️ Semi-payés: {alerts.filter(a => a.type === 'semi_payment').length}</span>
              <span style={{ color: '#dc3545' }}>🔴 Non-payés: {alerts.filter(a => a.type === 'unpaid').length}</span>
            </div>
            
            {alerts.map(alert => (
              <div key={alert.id} style={{ padding: '20px', borderBottom: '1px solid #eee', ...getAlertStyle(alert) }}>
                <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
                  <div style={{ fontSize: '24px' }}>{getAlertIcon(alert.type)}</div>
                  <div style={{ flex: 1 }}>
                    <h5 style={{ margin: 0 }}>
                      {alert.title}
                      {alert.fromDatabase && <span style={{ marginLeft: '10px', fontSize: '10px', background: '#17a2b8', color: 'white', padding: '2px 6px', borderRadius: '10px' }}>DB</span>}
                      <span style={{ marginLeft: '10px', fontSize: '11px', padding: '2px 8px', borderRadius: '10px', background: alert.type === 'semi_payment' ? '#ffc107' : '#dc3545', color: 'white' }}>
                        {alert.statusText || (alert.type === 'semi_payment' ? 'Semi-payé' : 'Non payé')}
                      </span>
                    </h5>
                    <p style={{ margin: '8px 0' }}>{alert.message}</p>
                    {alert.total !== undefined && (
                      <div style={{ display: 'flex', gap: '15px', fontSize: '13px', flexWrap: 'wrap' }}>
                        <span>💰 Total: <strong>{alert.total.toFixed(2)} DT</strong></span>
                        <span style={{ color: '#28a745' }}>✅ Payé: <strong>{alert.paid.toFixed(2)} DT</strong></span>
                        <span style={{ color: '#dc3545' }}>⚠️ Reste: <strong>{alert.amount.toFixed(2)} DT</strong></span>
                        {alert.pourcentage && <span>📊 {alert.pourcentage.toFixed(0)}% payé</span>}
                      </div>
                    )}
                    {(alert.phone || alert.address) && (
                      <div style={{ marginTop: '8px', fontSize: '12px', color: '#6c757d' }}>
                        {alert.phone && <span>📞 Tél: {alert.phone}</span>}
                        {alert.address && <span style={{ marginLeft: '15px' }}>📍 Adresse: {alert.address}</span>}
                      </div>
                    )}
                    <div style={{ display: 'flex', gap: '8px', marginTop: '10px', flexWrap: 'wrap' }}>
                      {alert.patientId && (
                        <button onClick={() => handlePaymentAction(alert.patientId)} style={{ padding: '5px 12px', background: '#28a745', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' }}>
                          <FaMoneyBillWave /> Gérer paiement
                        </button>
                      )}
                      {!alert.read && (
                        <button onClick={() => alert.fromDatabase && alert.dbId ? markDbAlertAsRead(alert.id, alert.dbId) : markAsRead(alert.id)} style={{ padding: '5px 12px', background: '#17a2b8', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' }}>
                          <FaCheckCircle size={12} /> Marquer lu
                        </button>
                      )}
                      <button onClick={() => sendReminder(alert)} style={{ padding: '5px 12px', background: '#667eea', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' }}>
                        <FaEnvelope /> Rappel
                      </button>
                      <button onClick={() => deleteAlert(alert)} style={{ padding: '5px 12px', background: '#dc3545', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
                        <FaTrash />
                      </button>
                    </div>
                  </div>
