...
const ConsultationsSection = ({ patients }) => {
  ...
  const fetchAllConsultations = async () => {
    ...
    try {
      const response = await axios.get(
        `${getBackendUrl()}/api/consultations`,
        axiosConfig,
      );
      ...
      if (error.response?.status === 500) {
        toast.error(
          "La table consultations n'existe pas. Veuillez exécuter le script SQL.",
        );
      } else {
  ...
  const fetchPatientConsultations = async (patientId) => {
    ...
    try {
      const response = await axios.get(
        `${getBackendUrl()}/api/consultations/patient/${patientId}`,
        axiosConfig,
      );
  ...
  const handleEditConsultation = async () => {
    ...
    try {
      const response = await axios.put(
        `${getBackendUrl()}/api/consultations/${selectedConsultation.id}`,
        consultationForm,
        axiosConfig,
      );
      if (response.data.success) {
  ...
  const handleDeleteConsultation = async (id) => {
    if (window.confirm("Supprimer cette consultation ?")) {
      ...
      try {
        await axios.delete(
          `${getBackendUrl()}/api/consultations/${id}`,
          axiosConfig,
        );
              ...
              >
                ...
                <td style={{ padding: "12px", textAlign: "center" }}>
                  <span
                    style={{
                      background:
                        getConsultationCount(patient.id) > 0
                          ? "#d4edda"
                          : "#f8d7da",
                      color:
                        getConsultationCount(patient.id) > 0
            ...
            >
              <h4 style={{ margin: 0 }}>
                ...
                {selectedConsultation
                  ...
                  : `Nouvelle consultation - ${selectedPatient?.full_name || ""}`}
              </h4>
              <button
                onClick={() => {
                  setShowConsultModal(false);
